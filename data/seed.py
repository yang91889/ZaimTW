...
def seed_if_empty() -> None:
    ...
    with conn:
        ...
        if count == 0:
            conn.execute(
                "INSERT INTO AccountBook(name, currency) VALUES(?,?)",
                ("個人帳本", "TWD"),
            )
            conn.execute(
                "INSERT INTO Account(book_id, name, type, balance_init) VALUES(1, '現金包', 'cash', 2000)"
            )
            conn.execute(
                "INSERT INTO Account(book_id, name, type, balance_init) VALUES(1, '銀行卡', 'bank', 10000)"
            )
