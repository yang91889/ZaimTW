...
class CategorySelectScreen(Screen):
    """Full-screen Category Picker (i18n)."""
    def __init__(self, name: str, on_pick: Callable[[Dict], None],
                 categories: List[Dict], recents: List[int] | None = None, **kwargs):
        ...
        pad = MDBoxLayout(orientation="horizontal", padding=(dp(12), dp(6), dp(12), dp(6)))
        self.search = MDTextField(
            hint_text=_t("SEARCH_CATEGORY"),
            size_hint_x=1, mode="round", icon_left="magnify",
    ...
    def _add_section_header(self, text: str):
        box = MDBoxLayout(orientation="vertical", padding=(dp(12), dp(8), dp(12), dp(4)))
    ...
    def _add_item(self, cat: Dict):
        item = TwoLineAvatarIconListItem(text=cat["name"], secondary_text=cat.get("subtitle", ""))
    ...
    def _rebuild_list(self, query: str = ""):
        ...
        if q:
            cats = [c for c in cats if q in c["name"].lower() or q in c.get("subtitle", "").lower()]

        if not q and self.recents:
            recent_objs = [c for c in self.all_categories if c["id"] in self.recents]
            if recent_objs:
        ...
        if not cats:
            none = MDLabel(text=_t("NO_MATCHES"), halign="center", theme_text_color="Hint", padding=(0, dp(24)))
            wrap = AnchorLayout(anchor_x="center", anchor_y="center", size_hint_y=None, height=dp(80))
